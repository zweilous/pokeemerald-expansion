...
static const struct MonChoiceData sStarterChoices[9] = 
{
    ...
    [BALL_MIDDLE_FIRST]     = {SPECIES_POPPLIO, 5},

    [BALL_TOP_THIRD]        = {SPECIES_GROOKEY, 5},
    [BALL_TOP_FOURTH]       = {SPECIES_SCORBUNNY, 5},
    [BALL_MIDDLE_THIRD]     = {SPECIES_SOBBLE, 5},
...
{
    const struct MonChoiceData *choice = &sStarterChoices[sBirchCaseDataPtr->handPosition];
    u8 *evs = (u8 *) choice->evs;
    u8 *ivs = (u8 *) choice->ivs;
    u16 *moves = (u16 *) choice->moves;
    u8 ball = choice->ball;
    u8 nature = choice->nature;
    u8 abilityNum = choice->abilityNum;
    u8 gender = choice->gender;
    u8 level = choice->level;
    u8 i;

    if (level == 0)
        level = 5;
    if (ball == BALL_STRANGE || ball >= POKEBALL_COUNT)
        ball = BALL_POKE;
    if (nature == 0)
    ...
    if (gender == 0)
    {
        u8 genderRatio = gSpeciesInfo[choice->species].genderRatio;
        if (genderRatio == MON_GENDERLESS || genderRatio == MON_MALE || genderRatio == MON_FEMALE)
            gender = genderRatio;
        else
            gender = ((u8)Random() < genderRatio) ? MON_FEMALE : MON_MALE;
    }
    ...
    gSpecialVar_Result = BirchCase_GiveMonParameterized(choice->species, level, choice->item, ball, nature, abilityNum, gender, evs, ivs, moves, choice->ggMaxFactor, choice->teraType, choice->isShinyExpansion);
}
